    ...
    <>
      ...
      <div style={{ position:'fixed', top:'50%', left:'50%', transform:'translate(-50%, -50%)', backgroundColor:'var(--header-footer-background)', padding:'30px', borderRadius:'10px', maxWidth:'500px', textAlign:'center', zIndex:1100, pointerEvents:'auto', color:'var(--text-color)' }}>
        ...
        <div style={{ marginTop:'20px' }}>
          <button onClick={() => router.push('/')} style={{ marginRight:'10px', padding:'8px 16px', borderRadius:'6px', cursor:'pointer', backgroundColor:'var(--button-background)', color:'var(--button-text)' }}>
            Go Home
          </button>
          {onRetry && (
            <button onClick={onRetry} style={{ padding:'8px 16px', borderRadius:'6px', cursor:'pointer', backgroundColor:'var(--button-background)', color:'var(--button-text)' }}>
              Retry
            </button>
          )}
        </div>
